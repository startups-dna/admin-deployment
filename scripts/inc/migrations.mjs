...
export async function runMigrations() {
  ...
  await execa({
    stdio: 'inherit',
  })`gcloud run jobs execute ${output.company?.dbJobName} --wait --args=npx,prisma,migrate,deploy --region=${config['gcp:region']} --project=${config['gcp:project']}`;

  if (output.appTools?.dbJobName) {
    echo.log('Running app-tools db migration...');
    await execa({
      stdio: 'inherit',
    })`gcloud run jobs execute ${output.appTools?.dbJobName} --wait --args=npx,prisma,migrate,deploy --region=${config['gcp:region']} --project=${config['gcp:project']}`;
  }
}
