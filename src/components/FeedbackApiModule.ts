...
export class FeedbackApiModule extends pulumi.ComponentResource {
  ...
  constructor(args: FeedbackApiModuleArgs, opts: pulumi.ComponentResourceOptions = {}) {
    super(`startupsdna:feedback:${FeedbackApiModule.name}`, PREFIX, args, opts);

    // get configuration
    const config = new pulumi.Config('feedbackApi');
    const serviceImage = config.get('serviceImage') || 'europe-west1-docker.pkg.dev/startupsdna-tools/admin-services/feedback-api:0.1.0';
    const domain = config.require('domain');
    const ipName = config.require('ipName');

    // create resources

    new gcp.projects.Service('gmail-api', {
    ...
    this.service = new gcp.cloudrunv2.Service(`${PREFIX}-service`, {
      ...
      template: {
        ...
        volumes: [
          { name: 'cloudsql', cloudSqlInstance: { instances: [args.database.sqlInstance.connectionName] } },
        ],
      },
    }, {
      parent: this,
    ...
    this.urlMap = new gcp.compute.URLMap(`${PREFIX}-url-map`, {
      hostRules: [{
        hosts: [domain],
    ...
    this.sslCert = new gcp.compute.ManagedSslCertificate(`${PREFIX}-ssl`, {
      managed: {
        domains: [domain],
      },
    }, {
    ...
    // Define HTTP proxies
    this.httpsProxy = new gcp.compute.TargetHttpsProxy(`${PREFIX}-target-https-proxy`, {
    ...
    new gcp.compute.GlobalForwardingRule(`${PREFIX}-forwarding-rule`, {
