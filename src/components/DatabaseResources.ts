...
export class DatabaseResources {
  ...
  constructor(name: string, args: DatabaseComponentArgs, opts: pulumi.ComponentResourceOptions = {}) {
    const { parent } = opts;
    this.sqlInstance = gcp.sql.DatabaseInstance.get(`${name}-sql-instance`, args.instanceId, {}, { parent });
    ...
    const db = new gcp.sql.Database(`${name}-db`, {
      name: args.dbName,
    ...
    const dbUser = new gcp.sql.User(`${name}-db-user`, {
      name: args.dbUser,
      instance: this.sqlInstance.name,
      type: 'BUILT_IN',
    ...
    this.dbUrlSecret = new gcp.secretmanager.Secret(`${name}-db-url-secret`, {
    ...
    this.dbUrlSecretVersion = new gcp.secretmanager.SecretVersion(`${name}-db-url-secret-version`, {
